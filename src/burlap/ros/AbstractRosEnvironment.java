 ...
 * abstract method {@link #getMostRecentRewardSignal(State, GroundedAction, State)} to be implemented.
 ...
 * methods:<p>
 * {@link #setActionPublisher(Action, burlap.ros.actionpub.ActionPublisher)},<p>
 ...
 * When {@link #executeAction(GroundedAction)} is called, the corresponding {@link burlap.ros.actionpub.ActionPublisher}
 * for the provided {@link GroundedAction} is retrieved (with a runtime exception being thrown if none have been set for it).
 * The {@link burlap.ros.actionpub.ActionPublisher#publishAction(GroundedAction)} is then called and passed the input
 * {@link GroundedAction}. After the method returns, the thread sleeps (and blocks) for the amount of time in
 ...
 * and after the thread sleeping completes for the post-state; this object's abstract {@link #getMostRecentRewardSignal(State, GroundedAction, State)}
 ...
 * then before the {@link #executeAction(GroundedAction)} method exits, it calls the
...
public abstract class AbstractRosEnvironment implements Environment {
	...
	/**
	 * Sets the {@link burlap.ros.actionpub.ActionPublisher} to handle executions of the given {@link Action}
	 * @param action the {@link Action} to handle
	...
	/**
	 * Sets a single {@link burlap.ros.actionpub.ActionPublisher} to handle the execution of a list of {@link Action} objects.
	 * @param actions the {@link Action} objects to handle
	...
	/**
	 * Generates the reward signal for the most recent environment interaction initiated through the {@link #executeAction(GroundedAction)}
	 * method and what will
	 * be returned by the {@link #lastReward()} method until another {@link #executeAction(GroundedAction)} method
	...
	/**
	 * This method is called just before exiting the {@link #executeAction(GroundedAction)} method
